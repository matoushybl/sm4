...
\begin{frame}
	...
	\movie[width=12cm,height=7cm,poster,autostart,repeat,showcontrols=true]{}{rail.mp4}
\end{frame}
...
\begin{frame}
	% nadpis snímku
	\frametitle{Demonstrace 2 - mapovací robot}
	\movie[width=12cm,height=7cm,poster,autostart,repeat,showcontrols=true]{}{mapbot.mp4}
\end{frame}
...
\frametitle{Otázky oponenta}
	\emph{Jak jste navrhl implementované regulátory?}\\[2ex]

	\begin{itemize}
		\item Rychlostní regulátor přemostěn kvůli absenci zpětné vazby. Rychlost je možné měřit podobně jako polohu, ale s nepoužitelně nízkým rozlišením.
		\item Poziční regulátor byl navržen metodou pokus omyl.
	\end{itemize}

	\emph{Postup návrhu regulátoru?}\\[2ex]
	\begin{itemize}
		\item Identifikace soustavy - v tomto případě generátoru ramp, dynamika motoru je vůči této zanedbatelná.
		\item Návrh pomocí klasických metod - kořenový hodograf, optimalizace pomocí kriteriálních funkcí.
	\end{itemize}
}

\frame{
	\frametitle{Otázky oponenta}
	\emph{Jaké rychlostní profily jste mohl implementovat?}\\[2ex]
	\begin{columns}[T] 								% prostředí sloupce s umístěním nahoře
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item Trapézoidní,
			...
			\begin{figure}%
				\centering
				\includegraphics[width=0.9\columnwidth]{obrazky/trap}
			\end{figure}
			Informace převzaty z webu trinamic.com
		\end{column}
		%
		\begin{column}{0.5\textwidth}		% druhý sloupec
			\begin{figure}%
				\centering
				\includegraphics[width=0.7\columnwidth]{obrazky/s_shape}
			\end{figure}
			\begin{figure}%
				\centering
				\includegraphics[width=0.7\columnwidth]{obrazky/six_pt}
			\end{figure}
		\end{column}
	\end{columns}
}
